...
```
text
  → 用 /(\([^)]*\)|[A-Za-z]+|\s+|[^A-Za-z()\s])/g 顺序匹配生成 token 流
  → 优先级：括号组 > 字母串 > 空白 > 其他字符
  → 对每个 (group)（非嵌套，取首个 ')'）：
      - 渲染字面 '('
      - 对 group 内容用 /([A-Za-z]+|\s+|[^A-Za-z\s])/g 再切
      - [A-Za-z]{2,} → 构造 <span class="word" data-w="原词">原词</span>
      - 其他内容（数字 / 1 字母 / 非拉丁 / 标点 / 空白）→ textNode
      - 渲染字面 ')'
  → 嵌套的 ( ... ( ...) ... )：内层括号里的单词也按上面规则处理
    （实现上：先扫描所有 ')' 边界，再对每个 ( 配对其右边界，处理其间内容）
  → 括号外的 [A-Za-z]+ 全部不处理，直接 textNode
```
...
2. **嵌套 / 多括号**：粘贴 `see (foo) and (bar (baz))` 确认两层括号都处理；`foo`、`bar`、`baz` 均被遮罩。
